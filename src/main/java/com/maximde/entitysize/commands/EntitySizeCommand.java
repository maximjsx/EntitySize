...
public class EntitySizeCommand implements CommandExecutor, TabCompleter {
            ...
            case "reset" -> {
                ...
                if (args.length == 1) {
                    ...
                    if (args[1].equalsIgnoreCase("@a")) {
                        ...
                        new ArrayList<>(Bukkit.getOnlinePlayers()).forEach(entitySize::resetSize);
    ...
    private boolean handleEntityArg(CommandSender sender, String[] args) {
        ...
        switch (args[1].toLowerCase()) {
            case "looking" -> {
                ...
                entitySize.getEntity(player, 30).thenAccept(optionalEntity -> {
                    ...
                    handleEntities(entity -> optionalEntity.get() == entity, size, finalTime, sender);
                });
    ...
    private void handleEntities(Predicate<Entity> condition, double size, int time, CommandSender sender) {
        EntitySize.scheduler().runNextTick(task -> {
            ...
            for (World world : Bukkit.getWorlds()) {
                ...
                try {
                    ...
                    entitySize.getLogger().warning("Could not enumerate entities in world '" + world.getName()
                            + "' for handleEntities: " + t.getClass().getSimpleName() + ": " + t.getMessage());
                    continue;
                }
    ...
    private void scheduleReset(List<UUID> entityUUIDs, int minutes) {
        EntitySize.scheduler().runLater(task -> {
